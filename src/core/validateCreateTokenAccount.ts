...
export async function validateCreateTokenAccount(
	...
	let lamports, space; // TODO: the checking should be impoved
	try {
		({ lamports: lamports, space: space } = SystemInstruction.decodeCreateAccount(instruction));
	} catch {
		throw new Error('invalid create token account instruction');
	}
